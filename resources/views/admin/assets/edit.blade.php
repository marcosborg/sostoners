    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="customer_id">{{ trans('cruds.asset.fields.customer') }}</label>
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $asset->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('customer'))
                    <span class="text-danger">{{ $errors->first('customer') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.customer_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="status_id">{{ trans('cruds.asset.fields.status') }}</label>
            ...
            <div class="form-group">
                <label for="monthly_value">{{ trans('cruds.asset.fields.monthly_value') }}</label>
                <input class="form-control {{ $errors->has('monthly_value') ? 'is-invalid' : '' }}" type="number" name="monthly_value" id="monthly_value" value="{{ old('monthly_value', $asset->monthly_value) }}" step="0.01">
                @if($errors->has('monthly_value'))
                    <span class="text-danger">{{ $errors->first('monthly_value') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.monthly_value_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="offers">{{ trans('cruds.asset.fields.offers') }}</label>
                <input class="form-control {{ $errors->has('offers') ? 'is-invalid' : '' }}" type="text" name="offers" id="offers" value="{{ old('offers', $asset->offers) }}">
                @if($errors->has('offers'))
                    <span class="text-danger">{{ $errors->first('offers') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.offers_helper') }}</span>
            </div>
            <div class="form-group">
